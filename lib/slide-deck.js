...
Write this as a literal, word-for-word script broken into 10-14 slides. Every "body" must be the EXACT words the presenter will say out loud into the camera — first person, natural spoken phrasing, contractions, direct address ("you," "your"). This is NOT a set of notes, outline points, or instructions about what to cover — never write things like "explain that..." or "mention how..."; write the actual sentence the presenter says. Everything you write will be displayed on screen and read verbatim, so do not include any presenter-only notes or delivery instructions — only words meant to be spoken.

Write in a natural, direct spoken voice. Avoid these AI-generated writing tells:
- Never use the reversal construction "it's not X, it's Y" or "that's not X, that's Y" (or close variants like "this isn't about X, it's about Y") — say the point plainly instead of setting up a false contrast to knock down.
...
The deck should open with a hook, build the argument to ${beliefGoal} step by step, and close with a natural, non-hypey mention of the knowledge-based tool hub category and MyToolHub — written in the same spoken style as everything else, not as an appended sales pitch.`;
}
