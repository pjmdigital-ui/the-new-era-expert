...
test('buildDiscoveryPrompt bans the AI-tell phrasing and fabricated-statistic patterns', () => {
  const prompt = buildDiscoveryPrompt('signal', ['x'], [], 3);
...
test('buildDiscoveryPrompt explains the pain-avoidance/pleasure-seeking angle split and asks for a genuine mix', () => {
  const prompt = buildDiscoveryPrompt('signal', ['x'], [], 3);
  assert.ok(prompt.includes('pain-avoidance'));
  assert.ok(prompt.includes('pleasure-seeking'));
  assert.ok(prompt.includes('do not default every topic to pain-avoidance'));
});
...
test('synthesizeCandidateTopics returns the parsed candidates array from a mock Claude response', async () => {
    ...
    return {
      ...
      json: async () => ({
        content: [{
          ...
          text: JSON.stringify({
            candidates: [
              { title: 'The knowledge hub category, explained in 90 seconds', entryPoint: 'category-exists', angle: 'pleasure-seeking', rationale: 'Surfaced from autocomplete signal for "knowledge hub"' },
            ],
          }),
        }],
      }),
  ...
  assert.equal(candidates[0].angle, 'pleasure-seeking');
  assert.ok(ANGLES.includes(candidates[0].angle));
});
