 ...
 *     and a single viral outlier shouldn't dominate the score)
 *   - Recency (videos inside RECENCY_WINDOW_DAYS get full weight, older
 *     ones decay linearly to a floor rather than dropping to zero)
 *   - Match count (more matching videos = more validated demand for the
 *     topic, with diminishing returns past ~10 results)
 *
...
function scoreFromSearchResults(videos, now = new Date()) {
  ...
  const viewComponent = videos.reduce((sum, v) => {
  ...
  const recencyComponent = videos.reduce((sum, v) => {
  ...
  // Combine and normalize into a roughly 0-100 range. The exact constants
  // here are a starting point, not tuned against real data yet — once
  // this runs against real topics, revisit against actual score
  // distributions rather than treating these as final.
  const raw = viewComponent * 3 + recencyComponent * 8 + matchComponent * 4;
