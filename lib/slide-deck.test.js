...
test('buildSlideDeckPrompt includes the topic title, the entry-point belief goal, the never-write-instructions requirement, and the phrasing/delivery-note/statistics rules', () => {
  ...
  assert.ok(prompt.includes('split across two sentences'));
  assert.ok(prompt.includes('how to deliver a line'));
  assert.ok(prompt.includes('Never invent numbers or data-sounding claims'));
});
        ...
        content: [{
          ...
          text: JSON.stringify({
            slides: [
              ...
              { headline: 'The pattern', body: 'x'.repeat(500) },
            ],
          }),
        }],
      }),
