 ...
 * behind the photo turns that same white background into a deliberate
 * "photo card" instead of a compositing bug.
 */
...
const FACE_PANEL_WIDTH = CANVAS_WIDTH * 0.34;
const FACE_PANEL_HEIGHT = CANVAS_HEIGHT * 0.86;
...
/**
 * Builds the presenter-photo panel: a white rounded-rect backing panel plus
 * the photo itself, clipped to the same rounded rect and cropped to fill it
 * (preserveAspectRatio="...slice"). Positioned as a right-side panel by
 * default — pair with buildThumbnailTextElement's centerX/maxWidth so the
 * headline text moves into the remaining left-hand space instead of
 * colliding with it.
 *
 * @param {Object} opts
...
function buildFaceLayerElements({
  ...
  return `<rect x="${x}" y="${y}" width="${width}" height="${height}" rx="${cornerRadius}" fill="#ffffff" />
